# 다독임 문서 안내

## 현재 상태

- 프로젝트: `다독임`
- GitHub 저장소: `https://github.com/SujinHuh/dadogim`
- 로컬 작업 범위: `/Users/huhsujin/Documents/Playground/개인프로젝트/다독임`만 기본 대상으로 사용
- 제품 관계: `inside-me`와 별개
- 핵심 기능: A. 수면 유도 소리, B. 긍정 확언, C. 멍 때리기 소리
- 진행 순서: `C → B → A`
- 현재 단계: C 요구사항 구체화
- 요구사항 완료 상태: `미완성`. `open-questions.md`가 미확정 사항의 임시 단일 목록이다.
- 최근 확정: C에서 `원하는 느낌으로 빠르게 추천받기`와 `전체 소리에서 직접 고르기`를 모두 제공한다.
- 앱 내부 구조: `기능 중심 폴더 + MVVM식 역할 분리 + 단방향 상태 흐름` (사용자 확정)
- PR 범위: 코드·기능은 작은 세로 조각으로 나누고, 문서 전용 변경은 같은 결정·검토 주제의 관련 문서와 로그를 한 묶음으로 검토한다.
- 로그 운영: DEC-032부터 `의도 → 검토·작업 → 결정·결과 → 해석 → 다음 행동`을 중심으로 짧게 기록하고 근거는 링크한다.
- PR 검수 체계: 깐깐한 관리자 검수는 요구사항·diff·실행 증거를 판단하고 병합은 사용자가 결정한다. PR 형식 Action은 코드와 첫 기능 일부가 생긴 뒤 도입 여부를 다시 결정한다.
- 토큰 절약 운영: 루트 지침은 짧게 유지하고, 작업별 경로 안내·제목/ID 검색·필요 범위 읽기·중복 금지를 기본 방식으로 확정했다.
- 현재 정리: C·B·A·공통·기술·비기능·시장 검증의 미확정 사항을 식별 가능한 체크리스트로 분리했다.
- 비관적 검수 결과: 요구사항은 아직 완성되지 않았으며, 미확정 항목을 확정 요구사항이나 구현 기준으로 사용할 수 없다.
- 시장 조사: 초기 경쟁 서비스 기능 조사 완료. 정량 시장·실사용 수요 검증은 미수행이다.
- 다음 사용자 확인: `open-questions.md`의 전체 미확정 목록. 확인 전에는 첫 항목 선택을 시작하지 않는다.

## 필요한 문서만 읽기

| 작업 | 우선으로 읽을 문서 |
| --- | --- |
| 제품 요구사항 논의 | `requirements.md`의 관련 기능 섹션 |
| 현재 미확정 항목 선택 | `open-questions.md`의 `현재 질문`만 |
| 경쟁 제품·차별화 확인 | `market-research.md`의 비관적 판단과 권장 방향 |
| 기존 결정 확인 | `decision-log.md`에서 관련 DEC 번호·제목 검색 |
| 최근 실제 변경 확인 | `change-log.md`의 관련 CHG 또는 마지막 항목 |
| PR 작성·크기·검증 기준 | `pull-request-guide.md` |
| 깐깐한 관리자 독립 검수 | `review-guide.md` |
| 개발·병렬화·리뷰·GitHub·CI/CD | `development-workflow.md`의 관련 섹션 |
| 전체 이력 감사 | 위 문서 전체 |

## 토큰 효율적인 문서 작성 규칙

- 불변 지침은 `AGENTS.md`, 현재 상태는 이 문서, 확정 요구사항은 `requirements.md`, 미확정 질문은 `open-questions.md`, 이력은 로그에 한 번씩만 쓴다.
- 새 작업은 이 문서의 현재 상태와 작업별 읽기 경로부터 확인하고, 제목과 DEC/CHG ID를 검색해 필요한 범위만 읽는다.
- 같은 턴에서 변경되지 않은 문서를 반복해서 읽지 않고, 상세 체크리스트는 실제로 적용할 작업에서만 연다.
- 원문 대화, 전체 터미널 출력, 중복 배경 설명을 복사하지 않고 `의도 + 검토·작업 + 결정·결과 + 해석 + 다음 행동`을 짧게 남긴다.
- 출처는 주장 가까이 한 번 링크하고, 같은 링크를 여러 로그에 반복하지 않는다.
- 문서가 커지면 서술을 지우기보다 상태 요약과 본문을 분리하고, 기본 작업에서는 요약만 읽는다.
- `decision-log.md`와 `change-log.md` 중 하나가 100개 항목 또는 약 100KB에 도달하면 ID와 원문을 바꾸지 않고 연도·분기별로 옮기며, 짧은 색인에는 ID·제목·상태·링크만 제공한다. 원문을 요약으로 대체하지 않는다. 임계값은 다독임 운영 기준이며 OpenAI의 공식 수치가 아니다.
- 기능별로 반복되는 별도 규칙이 생겼을 때만 해당 코드 가까이에 하위 `AGENTS.md`를 만들고 루트 규칙을 복사하지 않는다.

## 관련 공식 근거

- [Custom instructions with AGENTS.md](https://learn.chatgpt.com/ko-KR/docs/agent-configuration/agents-md)
- [Codex memories](https://learn.chatgpt.com/ko-KR/docs/customization/memories)
- [OpenAI model guidance: leaner prompts](https://developers.openai.com/api/docs/guides/latest-model)
- [Automating repetitive work at OpenAI with Codex](https://developers.openai.com/blog/automating-repetitive-work-at-openai-with-codex)
